...
import type { WorkflowStage, WorkflowSubStage } from "@/types/workflow";
...
export function WorkflowEditor({ organizationId }: WorkflowEditorProps) {
  ...
  const [deletingStage, setDeletingStage] = useState<WorkflowStage | null>(
    null
  );
  ...
  const [editingSubStage, setEditingSubStage] =
    useState<WorkflowSubStage | null>(null);
  ...
  const handleEditStage = (stage: WorkflowStage) => setEditingStage(stage);
  const handleDeleteStage = (stage: WorkflowStage) => setDeletingStage(stage);
  ...
  const handleAddSubStage = (stage: WorkflowStage) =>
    setAddingSubStageTo(stage);
  const handleEditSubStage = (subStage: WorkflowSubStage) =>
    setEditingSubStage(subStage);
  const handleDeleteSubStage = (subStage: WorkflowSubStage) =>
  ...
  const calculateNextSubStageSequence = (
    ...
    if (!stage) return 1;
    return stage.workflow_sub_stages.length + 1;
  };
          ...
          <div className="space-y-4">
            {workflowStructure.map((stage, stageIndex) => (
              ...
              >
                ...
                {/* Sub-stages List */}
                {stage.workflow_sub_stages.length > 0 && (
                  <div className="ml-6 mt-3 space-y-2 border-l pl-4">
                    {stage.workflow_sub_stages
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Button
                              ...
                              disabled={
                                subStageIndex ===
                                  stage.workflow_sub_stages.length - 1 ||
                                reorderStageMutation.isPending ||
                                reorderSubStageMutation.isPending
                              }
                            >
