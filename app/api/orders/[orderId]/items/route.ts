...
async function getFirstWorkflowStep(
  ...
  return { stageId: firstStageId, subStageId: firstSubStageId };
}
...
export async function POST(
  ...
  try {
    ...
    console.log(`Inserted new item with ID: ${newItemId}`);

    // 5. INSERT into item_history
    const { error: historyInsertError } = await supabase
      .from("item_history")
      .insert({
        item_id: newItemId,
        stage_id: firstStageId,
        sub_stage_id: firstSubStageId,
        entered_at: new Date().toISOString(), // Record entry time
        user_id: userId, // User who performed the action (creation)
        organization_id: orgId,
      });

    if (historyInsertError) {
      // This is problematic as the item exists but history tracking failed.
      ...
      console.error(
        `Error inserting item_history for item ${newItemId}:`,
        historyInsertError
      );
      // Consider returning success but logging the history failure
      // return NextResponse.json({ message: 'Item added, but history logging failed', itemId: newItemId }, { status: 207 });
      throw new Error("Failed to record item history."); // Fail request for now
    }

    console.log(`Inserted initial history record for item ${newItemId}`);

    return NextResponse.json(
      { message: "Item added successfully", itemId: newItemId },
      { status: 201 }
    );
  } catch (error: unknown) {
    console.error(
