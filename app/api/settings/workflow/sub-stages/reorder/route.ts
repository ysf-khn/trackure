...
/*
 Database Function Example (reorder_workflow_sub_stage in PostgreSQL):

 CREATE OR REPLACE FUNCTION reorder_workflow_sub_stage(
 ...
 BEGIN
     -- Get the current item's sequence order and parent stage ID
     SELECT stage_id, sequence_order INTO v_parent_stage_id, v_current_sequence
     FROM workflow_sub_stages
     WHERE id = p_sub_stage_id AND organization_id = p_organization_id; -- Ensure org match

     IF NOT FOUND THEN
         RAISE EXCEPTION 'Sub-stage not found or does not belong to the organization.';
     END IF;

     -- Find the sibling to swap with within the same parent stage
     IF p_direction = 'up' THEN
         SELECT id, sequence_order INTO v_sibling_id, v_target_sequence
         FROM workflow_sub_stages
         WHERE organization_id = p_organization_id
           AND stage_id = v_parent_stage_id -- Crucial: Same parent stage
     ...
     UPDATE workflow_sub_stages
     SET sequence_order = v_target_sequence
     WHERE id = p_sub_stage_id;
 ...
 -- GRANT EXECUTE ON FUNCTION reorder_workflow_sub_stage(UUID, UUID, TEXT) TO authenticated;
